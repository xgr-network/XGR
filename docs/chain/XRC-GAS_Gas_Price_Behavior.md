...
**Important:** On XGR, `maxPriorityFeePerGas` is **not** used as a dedicated “validator tip payout”.
If a user sets a non-zero priority fee, it can increase the **effective gas price** (and total fee),
but the resulting fee is still split by the fixed XGR formula (burn + donation + validator).
...
A: XGR returns a small suggested priority fee by default (e.g. 1 gwei) for wallet compatibility. Tips are not required under normal conditions.
If a non-zero tip is specified, it can increase the effective gas price (and total fee). Fee distribution still follows the fixed XGR split (not a dedicated tip payout).
